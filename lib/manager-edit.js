...
function pruneRank(markdown, id) {
  if (!findTable(markdown, "order")) return markdown;
  const rows = readRows(markdown, "order").filter((row) => row.id !== id);
  return replaceRows(markdown, "order", rows);
}
...
const APPLY = {
  ...
  deleteTask: (markdown, op) => pruneRank(deleteRow(markdown, "tasks", op.id), op.id),
  addEvent: (markdown, op) => addRow(markdown, "events", op.fields),
  updateEvent: (markdown, op) => updateRow(markdown, "events", op.id, op.fields),
  deleteEvent: (markdown, op) => pruneRank(deleteRow(markdown, "events", op.id), op.id),
